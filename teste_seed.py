...
# -------------------------
# Dados CIFAR-10 (carrega uma vez)
# -------------------------
(x_train, y_train), (x_test, y_test) = cifar10.load_data()
x_train = x_train.astype("float32")/255.0
x_test  = x_test.astype("float32")/255.0

# Índices de usuários por AP (fixo, não depende da seed)
ap_users = {
...
# ----------------------------------------------------------
# FUNÇÃO: roda UM experimento completo para uma dada seed
# ----------------------------------------------------------
def run_experiment(seed: int):
    print(f"\n===== INICIANDO EXPERIMENTO COM SEED {seed} =====")

    # 1) Sementes / determinismo
    set_all_seeds(seed)

    # 2) Dados (divisão por usuário, depende da seed)
    ...
    # 3) Modelo global e bandit
    bandit = BanditSelector(ap_users, ema_alpha=0.25)
    global_model = build_model()

    # ---- MÉTRICAS FIXAS DO MODELO (1x por experimento) ----
    ...
    print(f"\n🎯 Acurácia final do modelo global (seed={seed}): {acc * 100:.2f}%")
    ...
    # retorna métricas chave para agregação entre seeds
    return {
        "seed": seed,
        "acc_final": float(acc),
        "jain": float(jain_part),
        "total_flops": float(total_flops_all_rounds),
        "total_bytes": float(total_bytes_all_rounds),
    }
...
if __name__ == "__main__":
    SEEDS = list(range(10))   # 0,1,...,9  (10 seeds)
    results = []

    for s in SEEDS:
        print("\n############################################")
        print(f"##########  EXPERIMENTO SEED = {s}  ##########")
        print("############################################\n")
        res = run_experiment(s)
        results.append(res)

    # Resumo estatístico
    accs = np.array([r["acc_final"] for r in results])
    jains = np.array([r["jain"] for r in results])

    print("\n======== RESUMO FINAL SOBRE 10 SEEDS =========")
    print("Seeds:", SEEDS)
    print(f"Acurácia média ± dp: {accs.mean():.4f} ± {accs.std():.4f}")
    print(f"Jain médio ± dp:     {jains.mean():.4f} ± {jains.std():.4f}")
    print("==============================================")
